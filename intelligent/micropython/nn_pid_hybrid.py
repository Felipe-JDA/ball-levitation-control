# ═══════════════════════════════════════════════════════════════
# CONTROLLER: NEURAL NETWORK + PID HYBRID (INVERSE CORRECTION)
# Plant: Ball levitation | Hardware: ESP32 + HC-SR04 + PWM fan
# Platform: MicroPython (ulab)
#
# Combines a fixed PID Type 2 (feedback) with a Neural Network
# inverse model (feedforward) for correction. The RLS estimator
# learns the plant model online and the NN computes a feedforward
# term that complements the PID output.
# ═══════════════════════════════════════════════════════════════
...
U_MIN    = -30.0
U_MAX    =  30.0
PWM_LOW  = 172        # PWM lower bound
PWM_HIGH = 186        # PWM upper bound

# ── FIXED PID TYPE 2 PARAMETERS ────────────────────────────
theta_pid = np.array([-1.554093, 0.539169, -0.014852, -0.004281])
PID_POLE = 0.40

c0_pid = 0.0; c1_pid = 0.0; c2_pid = 0.0; c3_pid = 0.0
...
# Initial RLS weight vector
w_nn  = np.array([1.267258, 0.123666, -0.391469, -0.001234, -0.000605, -0.001864])
w_nn1 = np.array(w_nn)
...
def compute_pid():
    global c0_pid, c1_pid, c2_pid, c3_pid
    a1 = float(theta_pid[0]); a2 = float(theta_pid[1])
    b0 = float(theta_pid[2]); b1 = float(theta_pid[3])
    p  = PID_POLE
    ...
    try:
        ...
        c0_pid = float(X[0]); c1_pid = float(X[1])
        c2_pid = float(X[2]); c3_pid = float(X[3])
    except: pass
...
def run_forward_nn(yk):
    ...
    if current_cycle <= WARMUP_CYCLES: return ye
...
# ── COMBINED CONTROLLER (PID + NN FEEDFORWARD) ─────────────
def run_controller(yk, ye):
    global u, u_pid, u_ff, ek, ek_1, ek_2
    global uk_1, uk_2, uk_3, yk_1, yk_2, yk_3
    ...
    if current_cycle <= WARMUP_CYCLES:
        u = 0.0; u_pid = 0.0; u_ff = 0.0
        pwm_8bit = int(map_value(u, U_MIN, U_MAX, PWM_LOW, PWM_HIGH))
    ...
    # PID Type 2 (feedback)
    u_pid = (-(c3_pid - 1) * uk_1 + c3_pid * uk_2
             + c0_pid * ek + c1_pid * ek_1 + c2_pid * ek_2)

    # NN Inverse Model (feedforward)
    if abs(w4) > W4_MIN:
        u_ff = (ref - w1*yk - w2*yk_1 - w3*yk_2 - w5*uk_1 - w6*uk_2) / w4
    else:
        u_ff = 0.0

    u = constrain(u_pid + u_ff, U_MIN, U_MAX)
...
def print_serial(yk, ye):
    print(
        "ref:{:.1f} yk:{:.2f} ye:{:.2f} ek:{:.2f} "
        "u:{:.2f} u_pid:{:.2f} u_ff:{:.2f} "
...
def main():
    ...
    print("")
    print(" Controller: PID + NN Inverse Correction")
    print("")
