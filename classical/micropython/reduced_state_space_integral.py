"""
CONTROLLER: REDUCED STATE-SPACE WITH INTEGRAL ACTION
...
# Original = -0.682385,0.451019,-0.849815,-0.165791,0.262661,-0.186941
theta  = np.array([-2.946685,1.219947,0.752111,-0.153917,0.180153,-0.085176])
...
def compute_controller():
    ...
    A2 = np.dot(A_aug, A_aug)
    A3 = np.dot(A2, A_aug)
    A4 = np.dot(A3, A_aug)
    phi_c = A4 + A3 * al1 + A2 * al2 + A_aug * al3 + I4 * al4

    AB  = np.dot(A_aug, B_aug)
    A2B = np.dot(A_aug, AB)
    A3B = np.dot(A_aug, A2B)
    Co = np.zeros((4, 4))
    for i in range(4):
        Co[i][0] = B_aug[i];  Co[i][1] = AB[i]
        Co[i][2] = A2B[i];    Co[i][3] = A3B[i]

    try:
        Co_inv = np.linalg.inv(Co)
        e4 = np.array([0.0, 0.0, 0.0, 1.0])
        K_aug = np.dot(np.dot(e4, Co_inv), phi_c)
    except:
...
def run_controller(yk, ye):
    ...
    x_aug = np.array([yk, float(x_red[0]), float(x_red[1]), v_integral])
    u = -float(np.dot(K_aug, x_aug))

    if u > U_MAX:
        u = U_MAX;  v_integral -= ek
    elif u < U_MIN:
        u = U_MIN;  v_integral -= ek
...
def print_serial(yk, ye):
    print("ref:{:.1f} yk:{:.2f} ye:{:.2f} ek:{:.2f} u:{:.2f} vi:{:.2f} estado:{} "
          "theta:[{:.6f},{:.6f},{:.6f},{:.6f},{:.6f},{:.6f}]".format(
        ref, yk, ye, ek, u, v_integral, estado,
...
def main():
    print("=" * 50)
    print("  EE Reducido + Integral — MicroPython")
